...
mutable struct ChandeKrollStop{Tohlcv,S} <: TechnicalIndicator{Tohlcv}
    ...
    function ChandeKrollStop{Tohlcv,S}(; atr_period = ChandeKrollStop_ATR_PERIOD, atr_mult = ChandeKrollStop_ATR_MULT, period = ChandeKrollStop_PERIOD) where {Tohlcv,S}
...
function OnlineStatsBase._fit!(ind::ChandeKrollStop, candle)
    ...
    fit!(ind.high_stop_list, max([cdl.high for cdl in ind.input.value]...) - value(ind.atr) * ind.atr_mult)
    fit!(ind.low_stop_list, min([cdl.low for cdl in ind.input.value]...) + value(ind.atr) * ind.atr_mult)
    ...
    ind.value = ChandeKrollStopVal(max(ind.high_stop_list.value...),
