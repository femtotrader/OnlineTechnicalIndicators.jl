    ...
    if !ismissing(ind.swing_start_price) && !ismissing(ind.swing_end_price)
        ...
        if swing_size > 0
            ...
            if ind.swing_direction == :up
                ...
                retracement_38_long =
                    ind.swing_end_price -
                    (ind.swing_end_price - ind.swing_start_price) * ind.retracement_pct

                # Calculate current retracement from swing high
                current_retracement =
                    (ind.swing_end_price - current_bar.close) /
                    (ind.swing_end_price - ind.swing_start_price)
                ...
                retracement_38_short =
                    ind.swing_end_price +
                    (ind.swing_start_price - ind.swing_end_price) * ind.retracement_pct

                # Calculate current retracement from swing low
                current_retracement =
                    (current_bar.close - ind.swing_end_price) /
                    (ind.swing_start_price - ind.swing_end_price)
                current_retracement_pct = current_retracement
