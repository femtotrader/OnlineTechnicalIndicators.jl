...
mutable struct DPO{Tval} <: TechnicalIndicator{Tval}
    ...
    function DPO{Tval}(; period = DPO_PERIOD, ma = SMA) where {Tval}
        ...
        new{Tval}(missing, 0, period, sub_indicators, input_values)
    end
end
...
function OnlineStatsBase._fit!(ind::DPO, data)
    ...
    if length(ind.input_values) >= semi_period + 2 && length(ma.input_values) >= 1
        ind.value = ind.input_values[end-semi_period-1] - value(ma)
    else
        ind.value = missing
    end
end
